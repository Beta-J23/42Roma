...
/*   Updated: 2023/04/20 21:56:54 by alpelliz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
	...
	*/
	...
	while (i < a->size_of_int)
	{
		...
		//number_a = smallest_number_mod(a, b, pos_a); //trova nello stack_A numero corrispondente all'index pos_a
		//number_a = array_comparator(a, b, pos_a);
		...
		if (pos_b <= z)
			minimum_b = pos_b;
		tmp[i] = (minimum_a + minimum_b);
		ft_printf("number_a = %d, minimum A = %d, minimum B = %d, i = %d, tmp[i] = %d\n", number_a, minimum_a, minimum_b, i, tmp[i]);
		array_printer(a, b);
		pos_a++;
		j++;
		i++;
