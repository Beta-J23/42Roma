...
/*   Updated: 2023/03/30 15:24:30 by alpelliz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int draw_line_rep(void *mlx, void *win, int beginX, int beginY, int endX, int endY, int color)
{
	double pixelX = beginX;
	double pixelY = beginY;
	double deltaX = endX - beginX; 
...
/*
int draw_line2(void *mlx, void *mlx_win, int startX, int endX, int color)
{
	int	pixelX = startX;
	int	deltaX = endX - startX;
	while (deltaX)
	{
		mlx_pixel_put(mlx, mlx_win, deltaX, 1, color);
		--deltaX;
	}	
	return(0);
}
...
int deal_key(int key, t_position *pos)
{
	...
	{	
		...
		//draw_line(pos->mlx, pos->mlx_win, pos->beginX, pos->beginY, pos->endX + 1, pos->endY, 0x000000);
		draw_line(pos->mlx, pos->mlx_win, pos->beginX + pos->endX, pos->beginY, pos->endX, pos->endY, 0x000000);
	...
	else if (key == 124) //dx
	{
		...
		pos->endX += 10;
		draw_line(pos->mlx, pos->mlx_win, pos->beginX, pos->beginY, pos->endX, pos->endY, pos->color);
		draw_line(pos->mlx, pos->mlx_win, pos->beginX, pos->beginY, pos->endX - pos->beginX , pos->endY, 0x000000);
		ft_putchar('R');
	}
	else if (key == 126) //up
	{
		pos->beginY -= 10;
		pos->endY -= 10;
		//draw_line(pos->mlx, pos->mlx_win, pos->beginX, pos->endX, pos->endX, pos->endX, 0x000000);
		draw_line(pos->mlx, pos->mlx_win, pos->beginX, pos->beginY, pos->endX, pos->endY, 0x000000);
		draw_line(pos->mlx, pos->mlx_win, pos->endX, pos->beginY, pos->endX + 1, pos->endY - 10, pos->color);
	//	draw_line(pos->mlx, pos->mlx_win, pos->endX, pos->beginY, pos->endX, pos->endY, 0x000000);
		//pos->endX = pos->beginX +10;
		ft_putchar('U');
	}
	else if (key == 125) //down
	{
		pos->beginY += 10;
		pos->endY += 10;
		draw_line(pos->mlx, pos->mlx_win, pos->endX + 1, pos->beginY + 10, pos->beginX, pos->endY, pos->color);
		//draw_line(pos->mlx, pos->mlx_win, pos->beginX, pos->beginY, pos->endX, pos->endY, pos->color);
		
		//draw_line(pos->mlx, pos->mlx_win, pos->beginX, pos->beginY, pos->endX, pos->endY, 0x000000)
		ft_putchar('D');
	}
	//draw_line2(pos->mlx, pos->mlx_win, pos->beginX, pos->endX, pos->color);
